...
statement: instruction 
    ...
    | LLB statements LRB {
        $$=$2;    
        vector<variate> var = scopes[scopes.size()-1].varies;
        while(type_check.size()!=var.size())
        {
            if(type_check[type_check.size()-1].type != 4) func_code.addCode("\taddl\t$4, %esp\n");
            type_check.pop_back();
        }
        scopes.pop_back();
        scopeid--;
...
instruction: type params SEMICOLON 
    {
        ...
        for(int i = 1;i < node->childNum();i++)
        {
            ...
            for(int j = temp.size();j < type_check.size();j++)
            {
                if(type_check[j].name == (child->nodeType==NODE_ASSIGN?child->getChild(0)->varName:child->varName))
                {
                    printf("Error : variate wrong\n");
                    scope(type_check, scopeid).output();
                    praseErr_flag = 1;
                    break;
                }
            }
            if(!praseErr_flag)
            {
                if(vtype == VAR_STR)
                {
                    ...
                    type_check.emplace_back(variate(child->getChild(0)->varName, ro_data.size()));
                    ro_data.emplace_back(child->getChild(1)->str_val);
                    if(for_flag_begin) tmpfor.emplace_back(tmpvariate(variate(child->getChild(0)->varName, ro_data.size()), forlevel));
                    goto EA;
                }
                type_check.emplace_back(variate($1->varType, child->nodeType==NODE_ASSIGN?child->getChild(0)->varName:child->varName));
                for(int j = 0;j < child->dim.size();j++)
                {
                    type_check[type_check.size()-1].dim.emplace_back(child->dim[j]);
                }
                if(for_flag_begin) tmpfor.emplace_back(tmpvariate(variate($1->varType, child->nodeType==NODE_ASSIGN?child->getChild(0)->varName:child->varName), forlevel));
            }
            EA:
                praseErr_flag = 0;
        }
        declear_flag = 0;
        for_flag_begin = 0;
        temp_dec_size = type_check.size() + 1;
    }
    | params SEMICOLON {
        TreeNode *node=new TreeNode(NODE_STMT);
...
printf: PRINTF SLB STRING COMMA args SRB 
    {
        ...
        func_code.addCode("\tsubl\t$"+to_string(type_check.size()*4)+", %ebp\n");
        func_code.addCode("\tpushl\t$STR"+to_string(ro_data.size()-1)+"\n");
        func_code.addCode("\tcall\tprintf\n");
        func_code.addCode("\taddl\t$"+to_string(type_check.size()*4)+", %ebp\n");
        ...
        while(it != type_check.rend())
        {
            if((*it).name == $$->varName)
            {
                ...
                if($$->varType == VAR_STR)
                    ...
                    code += "-" + to_string(4*(type_check.size()-i)) + "(%ebp)\n";
                if(while_flag)
                {
...
_ID: ID 
    {
        ...
        if($$->int_val == -1)
        {
            ...
            if(!declear_flag) $$->int_val = type_check.size() - i;
